...
def login(request):
    ...
    design3=designation.objects.get(designation="team leader")
    Adm1=designation.objects.get(designation="Admin")
    desig4=designation.objects.get(designation="tester")
    if request.method == 'POST':
        ...
        elif user_registration.objects.filter(email=request.POST['email'], password=request.POST['password'],designation=design.id).exists():
        ...
        elif user_registration.objects.filter(email=request.POST['email'], password=request.POST['password'],designation_id=desig4.id).exists():
...
def BRadmin_trainerstable(request):
    if 'Adm_id' in request.session:
        ...
        Adm = user_registration.objects.filter(id=Adm_id)
        Trainer = designation.objects.get(designation='Trainer')
        trainers_data=user_registration.objects.filter(designation=Trainer).order_by("-id")
...
def man_registration_form(request):
    ...
    if request.method == 'POST':
        ...
        qr.save(settings.MEDIA_ROOT + "/images"+"//" +str(y1.id) + "of.png")
        with open(settings.MEDIA_ROOT + "/images"+"//"+ str(y1.id) +"of.png","rb") as reopen:
                djangofile = File(reopen)
        ...
        qr1.save(settings.MEDIA_ROOT +  "/images"+"//" +str(y2.id) + "re.png")
        with open(settings.MEDIA_ROOT +  "/images"+"//"+ str(y2.id) + "re.png", "rb") as reopen:
                djangofile = File(reopen)
        ...
        qr2.save(settings.MEDIA_ROOT +  "/images"+"//" +str(y3.id) + "ex.png")
        with open(settings.MEDIA_ROOT +  "/images"+"//" + str(y3.id) + "ex.png", "rb") as reopen:
                djangofile = File(reopen)
                y3.expqr = djangofile
                y3.save()
        ...
        msg_success = "Registration successfully check your mail"
...
def proMAN_rejectproj(request,id):
    ...
    if 'prid' in request.session:
        ...
        if request.method == 'POST':
            ...
            pro_sts = project.objects.filter(id=id).update(user_reason= user_reason,status ='rejected')
...
def projectmanDev(request):
    if 'prid' in request.session:
        if request.session.has_key('prid'):
            ...
            variable = "dummy"
        pro = user_registration.objects.filter(id=prid)
        man = user_registration.objects.filter(projectmanager_id = prid).filter(~Q(tl_id="0")).order_by("-id")
...
def projectManager_tl(request):
    if 'prid' in request.session:
        ...
        p1 = p.id
        tlfil = user_registration.objects.filter(projectmanager_id=prid).filter(designation_id=p1).order_by("-id")
...
def tltaskstatus(request,id):
    if 'tlid' in request.session:
        ...
        if request.method == 'POST':
            ...
            task.save()
            base_url = reverse('tlprojecttasks', kwargs={'id': mem1.project_id})
            query_string = urlencode({'id': mem1.project_id})
            url = '{}?{}'.format(base_url, query_string)
        return redirect(url)
...
def TLtasks(request):
    if 'tlid' in request.session:
        if request.session.has_key('tlid'):
            ...
            variable = "dummy"
        mem = user_registration.objects.filter(id=tlid)
        task = project_taskassign.objects.filter(tl_id=tlid,developer_id__isnull = True).order_by("-id")
...
def TStask(request):
    if 'usernametsid' in request.session:
        ...
        if request.session.has_key('usernametsid'):
            ...
            usernamets1 = "dummy"
        mem=user_registration.objects.filter(designation_id=usernamets) .filter(fullname=usernamets1)
        tasks = project_taskassign.objects.filter(developer=usertsid).order_by("-id")
...
def DEVtaskmain(request):
    if request.session.has_key('devid'):
        ...
        variable = "dummy"
    dev = user_registration.objects.filter(id=devid)
    task = project_taskassign.objects.filter(developer_id=devid).order_by("-id")
...
def man_registration_update(request,id):
    ...
    xem = extracurricular.objects.get(user_id=mem4.id)
    qem = qualification.objects.get(user_id=mem4.id)
    
    return render(request,'man_registration_update.html',{'mem4':mem4,'qem':qem})
